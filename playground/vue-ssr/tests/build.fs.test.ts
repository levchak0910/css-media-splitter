...
describe.sequential("plain application building", () => {
  it("correctly process application building", async () => {
    ...
    execSync("pnpm -F=vue-ssr build")

    let files = await dir.read.files(distPath, { recursive: true })
    files = files.filter(f => !f.endsWith(".js") && !f.endsWith(".json"))

    expect(files).toStrictEqual([
      "client/index.css",
      "client/index.html",
      "client/assets/screen-and-minwidth-1000px__index.css",
      "client/assets/screen-and-minwidth-2000px__index.css",
    ])
